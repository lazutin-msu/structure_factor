...
def readfile(filename):
    ...
    frames = []
    conv = {'id': lambda x: int(x), 'type': lambda x: int(x), 'mol': lambda x: int(x), 'xu': lambda x: float(x), 'yu': lambda x: float(x), 'zu': lambda x: float(x), 'ix': lambda x: int(x),'iy': lambda x: int(x),'iz': lambda x: int(x),'c_poten': lambda x: float(x),'c_bonen': lambda x: float(x)}
            ...
            if not line.startswith('ITEM: NUMBER OF ATOMS'):
                ...
                if not line.startswith('ITEM: BOX BOUNDS pp pp pp'):
                    ...
                    if not line.startswith('ITEM: ATOMS'):
                        ...
                        #print(atoms2)
            d1 = {'timestep':framenum, 'xlo':float(xlo), 'xhi':float(xhi), 'ylo':float(ylo), 'zhi':float(zhi), 'natoms': atomnum, 'atoms' : atoms2 }
            #print(framenum)
...
def readdata(filename):
    ...
    while iline<len(lines):
        line = lines[iline]
        line = line.lstrip() 
        m = re.search('^(-?\d+\.?\d*[Ee]?[+-]?\d*)\s+(-?\d+\.?\d*[Ee]?[+-]?\d*)\s+xlo\s+xhi$', line)
        if not m:
            iline += 1
            continue
        else:
    ...
    line = lines[iline]
    line = line.lstrip() 
    m = re.search('^(-?\d+\.?\d*[Ee]?[+-]?\d*)\s+(-?\d+\.?\d*[Ee]?[+-]?\d*)\s+ylo\s+yhi$', line)
    if m:
        ylo = float(m.group(1))
        yhi = float(m.group(2))
    else:
    ...
    line = lines[iline]
    line = line.lstrip() 
    m = re.search('^(-?\d+\.?\d*[Ee]?[+-]?\d*)\s+(-?\d+\.?\d*[Ee]?[+-]?\d*)\s+zlo\s+zhi$', line)
    if m:
        zlo = float(m.group(1))
        zhi = float(m.group(2))
    else:
...
def structure_factor_cuda_better_wrap2_progress(rf,qs,phi_num,theta_num,fmean):
  ...
  # sintheta = sintheta + 0.5 *sintheta_step
  # # theta = np.arcsin(sintheta)
  # theta = np.arccos(sintheta)
  theta, theta_step = np.linspace(np.pi, 0, num = theta_num, endpoint=False, retstep=True)
  theta = theta + 0.5* theta_step
    
  ptp, ptt = np.meshgrid(theta, phi)
  pt_pairs = np.dstack([ptp, ptt]).reshape(-1, 2)
...
def structure_factor_cuda_better_wrap2_progress_fibonacci(rf,qs,points_num,fmean):
...
def structure_factor_cuda_better_wrap2_progress_time(rf,qs,phi_num,theta_num,fmean):
  ...
  theta = np.arccos(sintheta)
  # theta, theta_step = np.linspace(-np.pi/2.0, np.pi/2.0, num = theta_num, endpoint=False, retstep=True)
  theta, theta_step = np.linspace(0, np.pi, num = theta_num, endpoint=False, retstep=True)
...
def structure_factor_cuda_better_wrap2_progress_time_cupy(rf,qs,phi_num,theta_num,fmean):
