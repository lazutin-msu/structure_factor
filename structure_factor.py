...
def structure_factor_cupy(rf,qs,points_num,fmean):
...
def structure_factor_cupy_directions(rf,qs,points_num,fmean):
    ...
    #res_abs,res_sq,res_qs = tr.structure_factor_cuda_better_wrap2_progress_cupy(rf, qs, 36, 18, 0.0)
     ...
     if args.gpu is not None:
      with cp.cuda.Device(args.gpu):
       if not args.diff:
           if not args.directions:
               res_abs,res_sq,res_qs = structure_factor_cupy(rf, qs, args.vec, 0.0)
           else:
               res_abs,res_sq,res_qs,xyz_all,xyz_sphere,phi_theta = structure_factor_cupy_directions(rf, qs, args.vec, 0.0)
       else:
           if not args.directions:
               res_abs,res_sq,res_qs = structure_factor_cupy(rf, qs, args.vec, f.mean())
           else:
               res_abs,res_sq,res_qs,xyz_all,xyz_sphere,phi_theta = structure_factor_cupy_directions(rf, qs, args.vec, f.mean())
     else:
       if not args.diff:
           if not args.directions:
               res_abs,res_sq,res_qs = structure_factor_cupy(rf, qs, args.vec, 0.0)
