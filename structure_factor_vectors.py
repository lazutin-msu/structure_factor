...
#           if not args.directions:
               res_abs,res_sq,res_qs = sf.structure_factor_cupy_everything(rf, qs, args.vec, 0.0, directions=False, print=False)
#           else:
...
#               res_abs,res_sq,res_qs,xyz_all,xyz_sphere,phi_theta = sf.structure_factor_cupy_everything(rf, qs, args.vec, 0.0, True)
    ...
    # else:
     ...
     res_abs_all_scalar.append(res_abs)
     res_sq_all_scalar.append(res_sq)
     res_qs_all_scalar.append(qs)

     peaks, prop = si.find_peaks(np.log(res_abs),prominence= args.prominence)
     
     peaks_all.append(peaks)
     proms_all.append(prop['prominences'])
     timesteps_all.append(frame['timestep'])

    peaks_unique = np.unique(np.concatenate(peaks_all))
    
    new_qs = qs[peaks_unique]
...
#           else:
               res_abs,res_sq,res_qs,xyz_all,xyz_sphere,phi_theta = sf.structure_factor_cupy_everything(rf, new_qs, args.vec, 0.0, directions=True, print=False)
     else:
#           if not args.directions:
#               res_abs,res_sq,res_qs = sf.structure_factor_cupy_everything(rf, qs, args.vec, 0.0, False)
#           else:
...
#     print('{:.1f} s used '.format(e-s), end='')
     ...
     if not np.array_equal(new_qs,res_qs):
    #   print('qs ok')
    # else:
       print('qs is not ok')
       print(new_qs)
     ...
     sf.printProgressBar(iframe+length+1, 2*length, prefix = 'Progress:', suffix = 'Complete', length = 50)
    ...
    res_abs_scalar = np.stack(res_abs_all_scalar,axis=0)
    res_sq_scalar  = np.stack(res_sq_all_scalar,axis=0)
    res_qs_scalar  = np.stack(res_qs_all_scalar,axis=0)
